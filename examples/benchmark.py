...
from sinkhorn.schedules import polynomial_schedule, constant_schedule
...
def main():
    print("="*70)
    print("Annealed Sinkhorn - 基本使用示例")
    print("="*70)
    ...
    config_standard = SinkhornConfig(
        ...
        beta_schedule=constant_schedule(beta=100.0)  # 固定高温度
    )
    
    # 配置2：Annealed Sinkhorn
    config_annealed = SinkhornConfig(
        algorithm_type=AlgorithmType.ANNEALED,
        num_iterations=num_iters,
        beta_schedule=polynomial_schedule(beta0, kappa=0.5)
    )
    
    # 配置3：Debiased Annealed Sinkhorn
    config_debiased = SinkhornConfig(
        algorithm_type=AlgorithmType.DEBIASED,
        num_iterations=num_iters,
        beta_schedule=polynomial_schedule(beta0, kappa=2/3),
        debiasing=True
    )
    ...
    ax.loglog(iters, metrics_std['ot_error'], 'o-', label='标准Sinkhorn', alpha=0.7)
    ax.loglog(iters, metrics_ann['ot_error'], 's-', label='Annealed Sinkhorn', alpha=0.7)
    ax.loglog(iters, metrics_deb['ot_error'], '^-', label='Debiased Annealed', alpha=0.7)
    
    # 理论斜率
    t_ref = np.linspace(50, num_iters, 50)
    ax.loglog(t_ref, 0.01 * (t_ref ** (-0.5)), 'k--', alpha=0.3, label='O(t^(-1/2))')
    ax.loglog(t_ref, 0.01 * (t_ref ** (-2/3)), 'k:', alpha=0.3, label='O(t^(-2/3))')
